...
import { quoteWooCommerceCheckout, type CheckoutQuote } from '../lib/checkoutQuote';
...
export function CartPage({ items, onUpdate, onComplete, onContinue }: Props) {
  ...
  const liveAccountRequired = requiresLiveCustomerAccount();
  ...
  useEffect(() => {
    if (!liveAccountRequired) return;
    let cancelled = false;
    getCustomerSession()
      .then((session) => { if (!cancelled) setCustomerSession(session); })
      .catch(() => { if (!cancelled) setCustomerSession({ authenticated: false }); });
    return () => { cancelled = true; };
  }, [liveAccountRequired]);
  useEffect(() => {
    if (!liveAccountRequired || !customerSession?.authenticated || !items.length) return;
    let cancelled = false;
    setQuoteState('loading'); setQuoteError(''); setCheckoutQuote(null);
    quoteWooCommerceCheckout(items).then((quote) => { if (!cancelled) { setCheckoutQuote(quote); setQuoteState('ready'); } }).catch((reason: unknown) => {
      if (!cancelled) { setQuoteError(reason instanceof Error ? reason.message : 'WooCommerce sepeti doğrulanamadı.'); setQuoteState('error'); }
    });
    return () => { cancelled = true; };
  }, [items, liveAccountRequired, customerSession?.authenticated]);
  ...
  return <main className="cart-page">
    ...
    {!items.length ? <section className="empty-cart"><ShoppingBag /><h2>Sepetin henüz boş.</h2><p>Koleksiyondan bir tasarım seçebilir veya stüdyoda kendi fikrini oluşturabilirsin.</p><button className="button button--primary" onClick={onContinue}>Koleksiyonu keşfet</button></section> : <div className="cart-layout">
      ...
      <aside className="checkout-card" aria-labelledby="checkout-title"><span className="eyebrow">TESLİMAT BİLGİLERİ</span>{liveAccountRequired && !customerSession ? <><h2 id="checkout-title">Hesap doğrulanıyor…</h2><p className="checkout-demo-note">Güvenli alışveriş oturumun kontrol ediliyor.</p></> : liveAccountRequired && !customerSession?.authenticated ? <><h2 id="checkout-title">Önce hesabına giriş yap.</h2><p className="checkout-demo-note">Canlı ödeme ve sipariş geçmişi için müşteri hesabı zorunludur.</p><Link className="button button--primary button--wide" to="/hesabim">Hesabımla devam et</Link></> : liveAccountRequired ? <><h2 id="checkout-title">WooCommerce sepet kontrolü</h2><p className="checkout-demo-note">Mağaza fiyatı ve varyant stoğu sunucudan doğrulanır. Gerçek sipariş ve ödeme henüz kapalıdır.</p>{quoteState === 'loading' && <p role="status">Ürün fiyatı ve stok kontrol ediliyor…</p>}{quoteState === 'error' && <p className="checkout-quote-error" role="alert">{quoteError}</p>}{quoteState === 'ready' && checkoutQuote && <div className="checkout-quote"><div><span>WooCommerce ürün ara toplamı</span><strong>{formatTRY(checkoutQuote.subtotal)}</strong></div><p>{checkoutQuote.note}</p></div>}</> : <><h2 id="checkout-title">Demo siparişini tamamla</h2><p className="checkout-demo-note">Ödeme alınmaz; gerçek sipariş veya dış sisteme mesaj gönderilmez.</p>
