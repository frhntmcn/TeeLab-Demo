      ...
      {!items.length ? (
        ...
        <div className="cart-layout">
          <section className="cart-items" aria-label="Sepet ürünleri">
            {items.map((item) => { const product = products.find((candidate) => candidate.id === item.productId); const stock = product && !item.isCustom ? getVariantStock(product, item.color, item.size) : undefined; return <article className="cart-line" key={item.id}>
              ...
              <div className="cart-quantity" aria-label={`${item.name} adedi`}><button onClick={() => onUpdate(item.id, item.quantity - 1)} aria-label="Adedi azalt"><Minus /></button><b>{item.quantity}</b><button disabled={stock !== undefined && item.quantity >= stock} onClick={() => onUpdate(item.id, item.quantity + 1)} aria-label="Adedi artır"><Plus /></button></div>
          ...
          <aside className="checkout-card">
            ...
            <form onSubmit={(event) => { event.preventDefault(); if (onComplete()) setCompleted(true); else setStockError(true); }}>
              ...
              {stockError && <p className="checkout-error" role="alert">Sepetindeki bir ürünün stoğu değişti. Lütfen adetleri güncelle.</p>}
