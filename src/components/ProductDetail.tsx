...
import type { Product, ShirtColor, ShirtSize, Side } from '../types';
import { ShirtVisual } from './Artwork';

export function ProductDetail({ product, onBack, onCustomize, onAdd }: { product: Product; onBack: () => void; onCustomize: () => void; onAdd: (qty: number) => void }) {
  ...
  const add = () => { onAdd(quantity); setAdded(true); window.setTimeout(() => setAdded(false), 2500); };
    ...
    <main className="detail-page">
      ...
      <div className="detail-layout">
        <div className="detail-visual">
          ...
          <ShirtVisual color={color} side={viewSide} artwork={viewSide === 'front' ? product.artwork : undefined} label={`${product.name} ${viewSide === 'front' ? 'ön' : 'arka'} görünümü`} />
        ...
        <section className="detail-panel">
          ...
          <strong className="detail-price">{(product.price * quantity).toLocaleString('tr-TR')} ₺</strong>
